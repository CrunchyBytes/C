...
int main(){
	...
	for (j=0; j<5; j++)
	{
        ...
        /* Inicialización de todo renglón en la sexta columna = 0
         * ventas[j][5] = 0;
         * ^También está correcto, sólo preferí inicializarla en el siguiente ciclo.
         */
    }
    
    /* Generación de semilla para números aleatorios a llenar la matriz 'ventas'
	...
	for (i=0; i<4; i++)
	{
		...
		for (j=0; j<5; j++)
		{
			...
			// Las ventas de cada renglón (tienda) se almacena en la sexta columna <--> [5] de 'ventas' de su renglón correspondiente.
